  ...
  const handleGenerateAIContent = async () => {
    if (!formData.nombre || !formData.categoria) {
      toast({
        ...
        description:
          "Por favor, ingresa el nombre y la categoría del negocio para generar contenido con IA.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const aiData = {
        ...
        descripcion: `Descubre ${
          formData.nombre
        }, el mejor lugar para ${formData.categoria.toLowerCase()} en Iztapa. Ofrecemos servicios de alta calidad y una experiencia inolvidable. ¡Visítanos!`,
      ...
      toast({
        title: "❌ Error de IA",
        description:
          "No se pudo generar el contenido con IA. Intenta de nuevo.",
        variant: "destructive",
      });
    } finally {
    ...
    >
      ...
      <div>
        ...
        <Input
          ...
          onChange={(e) =>
            handleSelectChange("services", e.target.value.split(","))
          }
        />
      </div>
      <div>
        <Label
          ...
          className="block text-sm font-medium mb-1"
        >
          URLs Galería Imágenes (separadas por coma)
        </Label>
        <Textarea
          id="gallery_images"
          name="gallery_images"
          ...
          onChange={(e) =>
            handleSelectChange("gallery_images", e.target.value.split(","))
          }
        />
      </div>
